"""FastAPI application for the SRP Long Straddle EMA Confirmation bot."""
...
async def lifespan(_: FastAPI):
    ...
    try:
        ...
        state.strategy_name = str(
            loader.get_strategy_config().get("name", "Long Straddle EMA Confirmation")
        )
        state.underlying = str(trading.get("underlying", ""))
        state.poll_interval = loader.get_polling_seconds()
        logger.info("SRP Long Straddle EMA Confirmation bot started")
        logger.info(
    ...
    bot.stop()
    reset_dhan_client()
    logger.info("SRP Long Straddle EMA Confirmation bot stopped")


app = FastAPI(
    title="SRP Dhan Long Straddle EMA Confirmation Bot",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def root() -> dict[str, Any]:
    ...
    loader = get_config_loader()
    return {
        "name": "SRP Dhan Long Straddle EMA Confirmation Bot",
        "status": get_bot_state().bot_status,
        "port": loader.get_port(),
        "endpoints": ["/", "/health", "/status", "/config", "/logs", "/stop"],
    }
...
async def status() -> dict[str, Any]:
    ...
    snapshot["ema"] = loader.get_ema_config()
    return snapshot
...
async def logs(lines: int = 100) -> PlainTextResponse:
    """Return the last N lines of the bot log."""
