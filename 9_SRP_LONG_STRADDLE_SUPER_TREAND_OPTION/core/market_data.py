"""Market data fetching via Dhan_SRP (spot, option LTPs, OHLC candles)."""
...
class MarketDataService:
    ...
    def fetch_spot(self, underlying: str | None = None) -> float | None:
        symbol = underlying or str(
            self.config_loader.get_trading_config().get("underlying", "NIFTY")
        )
    ...
    def fetch_latest_candle(
        self, underlying: str | None = None, timeframe: int = 5
    ) -> CandleBar | None:
        """Fetch latest intraday bar."""
    ...
    def fetch_candle_series(
        ...
        """Fetch OHLC series for Supertrend calculation."""
        trading = self.config_loader.get_trading_config()
        ...
        tf = (
            ...
            else int(st_cfg.get("timeframe_minutes", 5))
        )
        tf = tf if tf in {2, 3, 5, 10, 15, 30, 60} else 5
        need = min_bars if min_bars is not None else max(length * 5, 60)
        ...
        try:
            ...
            if hasattr(dhan, "get_intraday_data"):
                try:
                    ...
                    df = None
            if (df is None or getattr(df, "empty", True)) and hasattr(
