...
class MarketDataService:
    ...
    def _exchange_segment(self, instrument: dict, exchange: str) -> str:
        ...
        seg = str(trading.get("segment", "EQUITY")).upper()
        if seg == "OPTION":
    ...
    def _sync_today_daily_candle(
        ...
        Merge LTP into today's daily candle so EMA matches live charts.
        """
        ltp = self.fetch_ltp()
    ...
    def fetch_candles(self) -> CandleData | None:
        ...
        # Keep lookback modest for 1GB hosts while still converging EMA.
        if timeframe == "DAY":
            tail_bars = max(slow_ema * 15, 200)
        else:
            tail_bars = max(slow_ema * 10, 120)
