"""FastAPI server for the SRP Dhan MACD/RSI Trading Bot."""
...
from core.signal_engine import get_signal_engine, save_runtime_status
...
async def lifespan(_: FastAPI):
    ...
    try:
        ...
        logger.info("SRP Dhan MACD/RSI Trading Bot started")
        logger.info(
    ...
    engine.stop()
    reset_dhan_client()
    save_runtime_status(engine.state)
    logger.info("SRP Dhan MACD/RSI Trading Bot stopped")


app = FastAPI(title="SRP Dhan MACD/RSI Trading Bot", version="1.0.0", lifespan=lifespan)
...
async def root() -> dict[str, Any]:
    ...
    return {
        "name": "SRP Dhan MACD/RSI Trading Bot",
        "version": "1.0.0",
