...
class ConfigLoader:
    ...
    def get_strategy_mode(self) -> str:
        ...
        return str(self.get_strategy_config().get("mode", "MACD_RSI")).upper()
    ...
    def get_max_visible_polls(self) -> int:
        """Return number of poll blocks to show in rolling CLI dashboard."""
    ...
    def _validate(self, raw: dict[str, Any]) -> None:
        ...
        if mode in {"MACD", "MACD_RSI"}:
            if fast >= slow:
                raise ConfigError("macd.fast must be less than macd.slow")
            if signal_period <= 0:
                raise ConfigError("macd.signal must be greater than 0")

        rsi = raw.get("rsi", {})
        if mode in {"RSI", "MACD_RSI"}:
